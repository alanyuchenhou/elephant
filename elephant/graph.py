...
def configure_projector(model_dir, metadata_path):
    ...
    tensorboard.plugins.projector.visualize_embeddings(tensorflow.summary.FileWriter(model_dir),
...
def main(_):
    ...
    data_set = pandas.read_csv(
        os.path.join(os.path.dirname(__file__), "../resources/countryLevelCollaboration.tsv"),
        sep='\t',
        names=ATTRIBUTES,
        dtype={
    ...
    configure_projector(model_dir, os.path.join(os.path.dirname(__file__), '../resources/countries.tsv'), )
